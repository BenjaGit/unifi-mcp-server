...
|----------|-------------|----------|---------|
| `UNIFI_LOCAL_API_KEY` | Local key (UniFi Network Integrations) | Yes (for local mode) | - |
| `UNIFI_REMOTE_API_KEY` | Remote key (Site Manager API key from unifi.ui.com) | No | - |
| `UNIFI_PROTECT_API_KEY` | Protect API key | No | - |
...
docker run -i \
  --name unifi-mcp \
  -e UNIFI_REMOTE_API_KEY=your-unifi-remote-api-key \
...
- Provide `UNIFI_REMOTE_API_KEY` from [api.ui.com](https://api.ui.com/)
...
List all sites with aggregated statistics from the Site Manager API.

**Parameters:** None

**Returns:**
Array of site objects including health and inventory rollups.

**Example:**

```python
result = await mcp.call_tool("list_all_sites_aggregated", {})
```
...
### Additional Registered Read Tools

#### `list_dns_policies`

List DNS record override policies for a site.

**Parameters:**

- `site_id` (string, required): Site identifier
- `limit` (integer, optional): Maximum number of policies to return
- `offset` (integer, optional): Number of policies to skip

**Example:**

```python
result = await mcp.call_tool("list_dns_policies", {
    "site_id": "default",
    "limit": 50,
    "offset": 0
})
...
List switch port profiles for a site.

**Parameters:**

- `site_id` (string, required): Site identifier
- `limit` (integer, optional): Maximum number of profiles to return
- `offset` (integer, optional): Number of profiles to skip

**Example:**

```python
result = await mcp.call_tool("list_port_profiles", {
    "site_id": "default"
})
```
...
- `site_id` (string, required): Site identifier
- `limit` (integer, optional): Maximum number of tunnels to return
- `offset` (integer, optional): Number of tunnels to skip

**Example:**

```python
result = await mcp.call_tool("list_vpn_tunnels", {
    "site_id": "default"
})
```
...
Real-time traffic flow monitoring and analysis for UniFi Network 9.0+. Provides comprehensive network visibility and advanced analytics.
...
- **Troubleshooting**: Flow-level analysis for heavy users and hotspots
...
Resources use these URI schemes:
...
List all accessible UniFi sites.

**Example:**

```python
sites = await mcp.read_resource("sites://")
```
...
List traffic flows for a site.

**Example:**

```python
flows = await mcp.read_resource("sites://default/traffic/flows")
```

#### `site-manager://sites`

List sites from Site Manager API.

**Example:**

```python
sites = await mcp.read_resource("site-manager://sites")
```

#### `site-manager://health`

Get cross-site health metrics from Site Manager API.

**Example:**

```python
health = await mcp.read_resource("site-manager://health")
```

#### `site-manager://internet-health`

Get cross-site internet health metrics from Site Manager API.

**Example:**

```python
internet_health = await mcp.read_resource("site-manager://internet-health")
```
...
async def monitor_devices():
    ...
    devices = await mcp.call_tool("list_devices_by_type", {
        "site_id": "default",
        "device_type": "uap"
    })
...
Lists site events in reverse-chronological order (newest first).

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier (e.g. `"default"`) |
| `limit` | integer | No | Maximum events to return (default 100, max 3000) |
| `event_type` | string | No | Filter by event type prefix or exact key (e.g. `"EVT_AP"`, `"EVT_AP_Connected"`) |

**Returns:** `{ events: [...], count: N, site_id: "..." }`

Each event contains `key`, `datetime`, `msg`, `subsystem`, and device identifiers where applicable.

---

### `list_alarms`

Lists site alarms. Alarms are persistent notifications that require attention (e.g. an AP going offline).

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `limit` | integer | No | Maximum alarms to return (default 100) |
| `archived` | boolean | No | `true` = archived only, `false` = active only, omit = all |

**Returns:** `{ alarms: [...], count: N, site_id: "..." }`

---

### `archive_all_alarms`

Archives all active alarms for a site in a single operation. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without archiving |

**Returns:** `{ success: true }` or `{ dry_run: true, would_archive: "all" }`
...
Returns the health status of all site subsystems (WAN, LAN, WLAN, VPN). Includes CPU, memory, uptime, and ISP performance where available.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ subsystems: [...], count: N, site_id: "..." }`

Each subsystem entry includes `subsystem`, `status`, and subsystem-specific metrics such as `latency`, `xput_up`, `xput_down`, `num_user`, `num_ap`.

---

### `get_system_info`

Returns controller information: firmware version, build, hostname, IP addresses, timezone, and data retention settings.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** Flat object with fields including `version`, `build`, `hostname`, `timezone`, `autobackup`, `data_retention_days`.
...
Forces a device to re-provision immediately, pushing any pending configuration changes without waiting for the normal sync interval. Requires `confirm=true`.
...
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without provisioning |

**Returns:** `{ success: true, device_mac: "..." }` or dry-run preview.

---

### `run_speedtest`

Starts a WAN speed test on the gateway. Use `get_speedtest_status` to poll for results. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without running |

**Returns:** `{ success: true, message: "Speed test started. Use get_speedtest_status to check results." }`

---

### `get_speedtest_status`

Returns the current or most recently completed WAN speed test results. Read-only, no confirmation required.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** Speed test result object with `xput_download`, `xput_upload`, `latency`, `rundate`, `status`.

---

### `trigger_spectrum_scan`

Triggers an RF spectrum scan on a specific access point. The AP must be online and support spectrum scanning. Results are stored on the controller and can be retrieved via `get_spectrum_scan_results` on supported firmware. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `ap_mac` | string | Yes | Access point MAC address |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without scanning |

**Returns:** `{ success: true, ap_mac: "...", message: "Spectrum scan triggered on AP" }`
...
Lists all clients that have ever connected to the site, including offline clients. Returns historical data from `rest/user`. For currently active clients, use `list_active_clients` instead.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ clients: [...], count: N, site_id: "..." }`

---

### `forget_client`

Permanently removes a client from the controller's known client list. The client will be treated as completely new on its next connection. Requires `confirm=true`.
...
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without forgetting |

**Returns:** `{ success: true, client_mac: "..." }` or dry-run preview.

---

### `unauthorize_guest`

Revokes guest portal access for a client immediately. The client will need to re-authenticate through the captive portal. Requires `confirm=true`.
...
| `client_mac` | string | Yes | Client MAC address |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without revoking |

**Returns:** `{ success: true, client_mac: "...", message: "Guest access revoked" }` or dry-run preview.
...
**⚠️ Firmware Compatibility:** Not available on firmware 10.1.85 and earlier. Requires controller firmware 10.1.86+.

Configures the guest portal (captive portal) behavior for guest network access. Controls authentication method, session timeout, terms of service, and post-authentication redirect.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `portal_title` | string | No | Portal page title (e.g., "Welcome to Our WiFi") |
| `auth_method` | string | No | Authentication type: `"none"`, `"password"`, `"radius"` |
| `password` | string | No | Portal password (if auth_method is `"password"`) |
| `session_timeout` | integer | No | Session timeout in minutes (default 480 / 8 hours) |
| `redirect_enabled` | boolean | No | Enable post-login redirect to external URL |
| `redirect_url` | string | No | URL to redirect users after successful authentication |
| `terms_of_service_enabled` | boolean | No | Require acceptance of terms before portal access |
| `terms_of_service_text` | string | No | Custom terms of service text or HTML |

**Returns:** `{ success: true, portal: {...} }`
...
Lists all configured hotspot packages (paid WiFi plans) for the site.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ packages: [...], count: N, site_id: "..." }`
...
Retrieves details for a specific hotspot package.
...
Creates a new hotspot package for monetized guest WiFi access. Each package defines duration, bandwidth limits, data quota, and optional pricing. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `name` | string | Yes | Package name (e.g., "1 Hour - $5") |
| `duration_minutes` | integer | Yes | Session duration in minutes (e.g., 60, 1440) |
| `download_limit_kbps` | integer | No | Download bandwidth limit in Kbps (e.g., 5000 for 5 Mbps) |
| `upload_limit_kbps` | integer | No | Upload bandwidth limit in Kbps |
| `download_quota_mb` | integer | No | Total download quota in MB (null = unlimited) |
| `upload_quota_mb` | integer | No | Total upload quota in MB (null = unlimited) |
| `price` | number | No | Package price (currency from `currency` field) |
| `currency` | string | No | Currency code (e.g., `"USD"`, `"EUR"`; default `"USD"`) |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without creating |

**Returns:** `{ success: true, package_id: "...", package: {...} }`
...
Updates an existing hotspot package. Only specified fields are modified. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `package_id` | string | Yes | Hotspot package ID to update |
| `name` | string | No | New package name |
| `duration_minutes` | integer | No | New session duration in minutes |
| `download_limit_kbps` | integer | No | New download bandwidth limit |
| `upload_limit_kbps` | integer | No | New upload bandwidth limit |
| `download_quota_mb` | integer | No | New download quota |
| `upload_quota_mb` | integer | No | New upload quota |
| `price` | number | No | New package price |
| `enabled` | boolean | No | Enable or disable package |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without updating |
...
Deletes a hotspot package. Any active vouchers using this package will become invalid. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `package_id` | string | Yes | Hotspot package ID to delete |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without deleting |

**Returns:** `{ success: true, deleted_package_id: "..." }`
...
Lists all hotspot vouchers for the site. Vouchers are single-use codes that grant temporary guest WiFi access based on a package's duration and limits.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `limit` | integer | No | Maximum vouchers to return (default 100) |
| `offset` | integer | No | Pagination offset (default 0) |
| `filter_expr` | string | No | Optional filter expression (e.g., `"status=active"`) |

**Returns:** `{ vouchers: [...], count: N, site_id: "..." }`
...
Retrieves details for a specific voucher, including activation status, usage, and expiration.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `voucher_id` | string | Yes | Voucher ID |

**Returns:** `{ voucher: {...}, site_id: "..." }`
...
Generates one or more hotspot vouchers. Vouchers are single-use codes that guests can enter to activate temporary WiFi access. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `count` | integer | Yes | Number of vouchers to create (e.g., 10, 100) |
| `duration` | integer | Yes | Session duration in minutes (from package or custom) |
| `download_quota_mb` | integer | No | Total download quota in MB (null = unlimited) |
| `upload_quota_mb` | integer | No | Total upload quota in MB (null = unlimited) |
| `download_limit_kbps` | integer | No | Download bandwidth limit in Kbps |
| `upload_limit_kbps` | integer | No | Upload bandwidth limit in Kbps |
| `note` | string | No | Internal note (e.g., "Event on 2026-03-15") |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without creating |

**Returns:** `{ success: true, vouchers_created: N, vouchers: [{"code": "...", "activation_link": "..."}, ...] }`
...
Deletes a single voucher. Active sessions using this voucher will remain until expiration. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `voucher_id` | string | Yes | Voucher ID to delete |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without deleting |

**Returns:** `{ success: true, deleted_voucher_id: "..." }`
...
Bulk-deletes vouchers matching a filter expression. Use with caution — this operation cannot be undone. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `filter_expr` | string | Yes | Filter expression to match vouchers (e.g., `"status=unused"`, `"created<2026-01-01"`) |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without deleting |

**Returns:** `{ success: true, deleted_count: N, filter_used: "..." }`
...
Lists neighboring and rogue access points detected by the site's APs. Rogue APs are those detected on your own wired network. Neighboring APs are simply nearby SSIDs.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `within` | integer | No | Only include APs seen within this many hours (default 24) |

**Returns:** `{ aps: [...], count: N, rogue_count: N, site_id: "..." }`

Each AP entry includes `bssid`, `essid`, `channel`, `rssi`, `age`, and `is_rogue`.

---

### `list_available_channels`

Lists the RF channels available for the site's regulatory domain (country). Useful for checking which 2.4 GHz and 5 GHz channels are legal to use.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ channels: [...], count: N, site_id: "..." }`

---

## Firewall Groups

Firewall groups are named sets of IP addresses or ports that can be referenced in firewall rules, making rules easier to manage.

### `list_firewall_groups`

Lists all firewall groups for a site.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** Array of group objects, each with `_id`, `name`, `group_type`, `group_members`.

---

### `create_firewall_group`

Creates a new firewall group. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `name` | string | Yes | Group name |
| `group_type` | string | Yes | `"address-group"`, `"port-group"`, or `"ipv6-address-group"` |
| `group_members` | array | Yes | List of IPs/CIDRs (address groups) or port numbers (port groups) |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without creating |

**Returns:** Created group object or dry-run preview.

---

### `update_firewall_group`

Updates an existing firewall group's name or members. Only supplied fields are changed. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `group_id` | string | Yes | Group `_id` from `list_firewall_groups` |
| `name` | string | No | New name |
| `group_members` | array | No | Replacement member list (replaces entirely, not appended) |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without updating |

**Returns:** Updated group object or dry-run preview.

---

### `delete_firewall_group`

Deletes a firewall group. Any firewall rules referencing this group will lose their group binding. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `group_id` | string | Yes | Group `_id` |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without deleting |

**Returns:** `{ success: true, group_id: "..." }` or dry-run preview.

---

## Historical Reports & Sessions

### `get_historical_report`

Returns aggregated historical statistics for the site, an AP, or a specific client over a time range.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `interval` | string | Yes | Aggregation interval: `"5minutes"`, `"hourly"`, or `"daily"` |
| `report_type` | string | Yes | Report scope: `"site"`, `"ap"`, or `"user"` |
| `start` | integer | Yes | Start time as Unix epoch (seconds) |
| `end` | integer | Yes | End time as Unix epoch (seconds) |
| `attrs` | array | No | Specific attributes to include (default: `bytes`, `num_sta`, `time`) |

**Returns:** `{ data: [...], count: N, interval: "...", report_type: "...", site_id: "..." }`

---

### `list_sessions`

Lists client session history for the site. Returns all client sessions (including offline clients) within the time range. Defaults to the last 24 hours if no time range is provided.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `start` | integer | No | Start time as Unix epoch (default: 24 hours ago) |
| `end` | integer | No | End time as Unix epoch (default: now) |
| `limit` | integer | No | Cap on number of sessions returned |

**Returns:** `{ sessions: [...], count: N, site_id: "..." }`

Each session includes `mac`, `hostname`, `duration`, `rx_bytes`, `tx_bytes`, `ap_mac`, and `satisfaction`.

---

## Traffic Rules

Traffic rules use the v2 API to manage per-application or per-category rate limiting and blocking. **These tools only work with the local gateway API** (`UNIFI_API_TYPE=local`) and will raise an error when used with cloud APIs.

### `list_traffic_rules`

Lists all v2 traffic rules for a site.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** Array of traffic rule objects, each with `_id`, `description`, `action`, `enabled`, `matching_target`, and schedule.

---

### `create_traffic_rule`

Creates a new traffic rule. The rule data object must conform to the UniFi v2 traffic rule schema. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `rule_data` | object | Yes | Full traffic rule definition |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without creating |

**Returns:** Created rule object or dry-run preview.

---

### `update_traffic_rule`

Replaces a traffic rule's configuration entirely. Supply the full rule object (not just changed fields). Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `rule_id` | string | Yes | Rule `_id` from `list_traffic_rules` |
| `rule_data` | object | Yes | Full updated rule definition |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without updating |

**Returns:** Updated rule object or dry-run preview.

---

### `delete_traffic_rule`

Deletes a traffic rule. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `rule_id` | string | Yes | Rule `_id` |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without deleting |

**Returns:** `{ success: true, rule_id: "..." }` or dry-run preview.
...
Returns all site-level configuration settings. There are approximately 37 setting categories including `connectivity`, `ips`, `dpi`, `mgmt`, `snmp`, `rsyslogd`, and others.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `key` | string | No | Filter to a specific setting category (e.g. `"ips"`, `"connectivity"`) |

**Returns:** `{ settings: [...], count: N, site_id: "..." }`

Use this tool first to read the current setting object before calling `update_site_setting`.

---

### `update_site_setting`

Updates a specific site setting category. You must supply the full modified setting object — use `get_site_settings` to retrieve the current state first, modify the fields you want to change, then pass the entire object back. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `setting_key` | string | Yes | Setting category key (e.g. `"ips"`) |
| `setting_id` | string | Yes | The `_id` field from the setting object |
| `setting_data` | object | Yes | Full modified setting object |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without updating |

**Returns:** Updated setting object or dry-run preview.
...
Returns the current Dynamic DNS status for all configured DDNS providers on the gateway.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ ddns: [...], count: N, site_id: "..." }`

Each entry includes `service`, `hostname`, `ip`, `status`, and `last_error`.

---

### `list_active_routes`

Returns the gateway's active routing table.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ routes: [...], count: N, site_id: "..." }`

Each route includes `pfx` (prefix/destination), `nh` (next-hop array with `t` type, `via`, `intf`, and `metric`).
...
Lists user bandwidth groups. These groups can be assigned to clients to apply download/upload rate limits.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ groups: [...], count: N, site_id: "..." }`

---

### `list_wlan_groups`

Lists WLAN groups. WLAN groups are containers that hold one or more SSIDs and are assigned to APs.
...
Lists MAC address tags for the site. Tags can be used to group devices for policy targeting.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |

**Returns:** `{ tags: [...], count: N, site_id: "..." }`
...
Creates a new site on the controller. The `site_id` parameter identifies which site is used to authenticate; the new site is created at the controller level. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Authenticating site identifier |
| `name` | string | Yes | Short name for the new site (used in API paths) |
| `description` | string | Yes | Human-readable display name |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without creating |

**Returns:** Created site data or dry-run preview.

---

### `delete_site`

Permanently deletes a site and all its configuration. This operation is irreversible. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Authenticating site identifier |
| `target_site_name` | string | Yes | Name of the site to delete |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without deleting |

**Returns:** `{ success: true, site_name: "..." }` or dry-run preview.

---

### `move_device`

Moves a device from the current site to another site on the same controller. The device will re-provision under its new site. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Source site identifier |
| `device_mac` | string | Yes | Device MAC address |
| `target_site_id` | string | Yes | Destination site identifier |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without moving |

**Returns:** Result object or dry-run preview.

---

## System Control

These tools send commands directly to the gateway. Use with caution in production environments.

### `reboot_gateway`

Reboots the gateway. All clients will lose network connectivity for the duration of the reboot (typically 1–3 minutes). Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without rebooting |

**Returns:** `{ success: true, action: "reboot" }` or dry-run preview.

---

### `poweroff_gateway`

Powers off the gateway. **Physical access will be required to restart it.** Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without powering off |

**Returns:** `{ success: true, action: "poweroff" }` or dry-run preview.

---

### `clear_dpi_counters`

Resets all DPI (Deep Packet Inspection) traffic counters for the site to zero. Historical DPI data is not recoverable after this operation. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without clearing |

**Returns:** `{ success: true, action: "clear_dpi" }` or dry-run preview.
...
Migrates a device to a different UniFi controller by setting its inform URL. The device will disconnect from the current controller and attempt to connect to the new one. Requires `confirm=true`.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `site_id` | string | Yes | Site identifier |
| `device_mac` | string | Yes | Device MAC address |
| `inform_url` | string | Yes | Full inform URL of the target controller (e.g. `https://192.168.1.1:8080/inform`) |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without migrating |
...
Cancels an in-progress device migration, reverting the device's inform URL back to the current controller. Requires `confirm=true`.
...
| `device_mac` | string | Yes | Device MAC address |
| `confirm` | boolean | Yes | Must be `true` to execute |
| `dry_run` | boolean | No | Preview without cancelling |
...
### Known Limitations by Firmware Version

#### UniFi Network 10.1.85 and Earlier

The following tools are **not available** on firmware 10.1.85 due to missing API endpoints:

| Tool | Issue | Required | Workaround |
|------|-------|----------|-----------|
| `configure_guest_portal` | Endpoint `/proxy/network/integration/v1/.../guest-portal/config` returns 404 | Firmware 10.1.86+ | Upgrade controller firmware |
| `list_hotspot_packages` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `get_hotspot_package` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `create_hotspot_package` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `update_hotspot_package` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `delete_hotspot_package` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `list_vouchers` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `get_voucher` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `create_vouchers` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `delete_voucher` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |
| `bulk_delete_vouchers` | Guest/hotspot system not available | Firmware 10.1.86+ | Upgrade controller firmware |

**Recommendation:** Update your controller to the latest firmware version to access these tools.

### Parameter Clarifications

#### `set_device_port_overrides`

The `device_id` parameter accepts both:
- **MongoDB ObjectId format** (e.g., `"65da05a5abcd1234..."`) — Device identifier from legacy API
- **MAC address format** (e.g., `"f4:92:bf:a5:a1:f4"`) — Device MAC address

Both formats are accepted and will be automatically resolved to the correct device. For consistency with other tools, MAC address format is recommended.
...
**Last Updated:** 2026-03-14 | **API Version:** 0.2.4
