"""Device management MCP tools."""

from typing import Any

from fastmcp.server.providers import LocalProvider

from ..api.pool import get_network_client
from ..models import Device
from ..utils import (
    ResourceNotFoundError,
    audit_action,
    get_logger,
    validate_confirmation,
    validate_device_id,
    validate_limit_offset,
    validate_mac_address,
    validate_site_id,
)
from ._helpers import resolve, unwrap

logger = get_logger(__name__)
provider = LocalProvider()

__all__ = [
    "provider",
    "get_device_details",
    "get_device_statistics",
    "list_devices_by_type",
    "search_devices",
    "list_pending_devices",
    "adopt_device",
    "execute_port_action",
]


@provider.tool()
async def get_device_details(site_id: str, device_id: str) -> dict[str, Any]:
    """Get detailed information for a specific device.

    Args:
        site_id: Site identifier
        device_id: Device identifier

    Returns:
        Device details dictionary

    Raises:
        ResourceNotFoundError: If device not found
    """
    site_id = validate_site_id(site_id)
    device_id = validate_device_id(device_id)

    client, site = await resolve(site_id, get_network_client)

    # Get all devices and find the specific one
    response = await client.get(client.legacy_path(site.name, "devices"))
    devices_data = unwrap(response)

    for device_data in devices_data:
        if device_data.get("_id") == device_id:
            device = Device(**device_data)
            logger.info(f"Retrieved device details for {device_id}")
            return device.model_dump()

    raise ResourceNotFoundError("device", device_id)


@provider.tool()
async def get_device_statistics(site_id: str, device_id: str) -> dict[str, Any]:
    """Retrieve real-time statistics for a device.

    Args:
        site_id: Site identifier
        device_id: Device identifier

    Returns:
        Device statistics dictionary
    """
    site_id = validate_site_id(site_id)
    device_id = validate_device_id(device_id)

    client, site = await resolve(site_id, get_network_client)

    response = await client.get(client.legacy_path(site.name, "devices"))
    devices_data = unwrap(response)

    for device_data in devices_data:
        if device_data.get("_id") == device_id:
            # Extract statistics
            stats = {
                "device_id": device_id,
                "uptime": device_data.get("uptime", 0),
                "cpu": device_data.get("cpu"),
                "mem": device_data.get("mem"),
                "tx_bytes": device_data.get("tx_bytes", 0),
                "rx_bytes": device_data.get("rx_bytes", 0),
                "bytes": device_data.get("bytes", 0),
                "state": device_data.get("state"),
                "uplink_depth": device_data.get("uplink_depth"),
            }
            logger.info(f"Retrieved statistics for device {device_id}")
            return stats

    raise ResourceNotFoundError("device", device_id)


@provider.tool()
async def list_devices_by_type(
    site_id: str,
    device_type: str,
    limit: int | None = None,
    offset: int | None = None,
) -> list[dict[str, Any]]:
    """Filter devices by type (AP, switch, gateway).

    Args:
        site_id: Site identifier
        device_type: Device type filter (uap, usw, ugw, etc.)
        limit: Maximum number of devices to return
        offset: Number of devices to skip

    Returns:
        List of device dictionaries
    """
    site_id = validate_site_id(site_id)
    limit, offset = validate_limit_offset(limit, offset)

    client, site = await resolve(site_id, get_network_client)

    response = await client.get(client.legacy_path(site.name, "devices"))
    devices_data = unwrap(response)

    # Filter by type ("all" returns every device)
    if device_type.lower() == "all":
        filtered = list(devices_data)
    else:
        filtered = [
            d
            for d in devices_data
            if d.get("type", "").lower() == device_type.lower()
            or device_type.lower() in d.get("model", "").lower()
        ]

    # Apply pagination
    paginated = filtered[offset : offset + limit]

    # Parse into Device models
    devices = [Device(**d).model_dump() for d in paginated]

    logger.info(f"Retrieved {len(devices)} devices of type '{device_type}' for site '{site_id}'")
    return devices


@provider.tool()
async def search_devices(
    site_id: str,
    query: str,
    limit: int | None = None,
    offset: int | None = None,
) -> list[dict[str, Any]]:
    """Search devices by name, MAC, or IP address.

    Args:
        site_id: Site identifier
        query: Search query string
        limit: Maximum number of devices to return
        offset: Number of devices to skip

    Returns:
        List of matching device dictionaries
    """
    site_id = validate_site_id(site_id)
    limit, offset = validate_limit_offset(limit, offset)

    client, site = await resolve(site_id, get_network_client)

    response = await client.get(client.legacy_path(site.name, "devices"))
    devices_data = unwrap(response)

    # Search by name, MAC, or IP
    query_lower = query.lower()
    filtered = [
        d
        for d in devices_data
        if query_lower in d.get("name", "").lower()
        or query_lower in d.get("mac", "").lower()
        or query_lower in d.get("ip", "").lower()
        or query_lower in d.get("model", "").lower()
    ]

    # Apply pagination
    paginated = filtered[offset : offset + limit]

    # Parse into Device models
    devices = [Device(**d).model_dump() for d in paginated]

    logger.info(f"Found {len(devices)} devices matching '{query}' in site '{site_id}'")
    return devices


@provider.tool()
async def list_pending_devices(
    site_id: str,
    limit: int | None = None,
    offset: int | None = None,
) -> list[dict[str, Any]]:
    """List devices awaiting adoption on the specified site.

    Args:
        site_id: Site identifier
        limit: Maximum number of devices to return
        offset: Number of devices to skip

    Returns:
        List of pending device dictionaries
    """
    site_id = validate_site_id(site_id)
    limit, offset = validate_limit_offset(limit, offset)

    client = get_network_client()

    params = {"limit": limit, "offset": offset}

    # Pending devices is a global endpoint, not per-site
    response = await client.get(client.integration_base_path("pending-devices"), params=params)
    devices_data = unwrap(response)

    # Parse into Device models
    devices = [Device(**d).model_dump() for d in devices_data]

    logger.info(f"Retrieved {len(devices)} pending devices")
    return devices


@provider.tool()
async def adopt_device(
    site_id: str,
    mac_address: str,
    name: str | None = None,
    confirm: bool | str = False,
    dry_run: bool | str = False,
) -> dict[str, Any]:
    """Adopt a pending device onto the specified site.

    Uses the official Integration API: POST /v1/sites/{id}/devices
    with the device MAC address in the request body.

    Args:
        site_id: Site identifier
        mac_address: MAC address of the device to adopt (from list_pending_devices)
        name: Optional device name
        confirm: Confirmation flag (required)
        dry_run: If True, validate but don't execute

    Returns:
        Adopted device information
    """
    validate_confirmation(confirm, "adopt device", dry_run)
    site_id = validate_site_id(site_id)
    mac_address = validate_mac_address(mac_address)

    client, site = await resolve(site_id, get_network_client)

    payload: dict[str, Any] = {"macAddress": mac_address}
    if name:
        payload["name"] = name

    if dry_run:
        logger.info(f"[DRY RUN] Would adopt device {mac_address} with payload: {payload}")
        return {"dry_run": True, "mac_address": mac_address, "payload": payload}

    response = await client.post(client.integration_path(site.uuid, "devices"), json_data=payload)
    data = response.get("data", response)

    # Audit the action
    await audit_action(
        client.settings,
        action_type="adopt_device",
        resource_type="device",
        resource_id=mac_address,
        site_id=site_id,
        details={"name": name} if name else {},
    )

    logger.info(f"Successfully adopted device {mac_address}")
    return Device(**data).model_dump()


@provider.tool()
async def execute_port_action(
    site_id: str,
    device_id: str,
    port_idx: int,
    action: str,
    params: dict[str, Any] | None = None,
    confirm: bool | str = False,
    dry_run: bool | str = False,
) -> dict[str, Any]:
    """Execute an action on a specific port of a device.

    Args:
        site_id: Site identifier
        device_id: Device identifier
        port_idx: Port index number
        action: Action to perform (power-cycle, enable, disable)
        params: Additional action parameters
        confirm: Confirmation flag (required)
        dry_run: If True, validate but don't execute

    Returns:
        Action result
    """
    validate_confirmation(confirm, f"execute port action '{action}'", dry_run)
    site_id = validate_site_id(site_id)
    device_id = validate_device_id(device_id)

    client, site = await resolve(site_id, get_network_client)

    payload = {"action": action, "params": params or {}}

    if dry_run:
        logger.info(
            f"[DRY RUN] Would execute port action '{action}' on device {device_id} port {port_idx}"
        )
        return {
            "dry_run": True,
            "device_id": device_id,
            "port_idx": port_idx,
            "payload": payload,
        }

    response = await client.post(
        client.integration_path(
            site.uuid, f"devices/{device_id}/interfaces/ports/{port_idx}/actions"
        ),
        json_data=payload,
    )
    data = response.get("data", response)

    # Audit the action
    await audit_action(
        client.settings,
        action_type="port_action",
        resource_type="device_port",
        resource_id=f"{device_id}:{port_idx}",
        site_id=site_id,
        details={"action": action},
    )

    logger.info(f"Successfully executed port action '{action}' on port {port_idx}")
    return {"success": True, "action": action, "port_idx": port_idx, "result": data}
