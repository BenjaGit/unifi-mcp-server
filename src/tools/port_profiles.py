...
async def list_port_profiles(
    ...
    response = await client.get(client.legacy_path(site.name, "rest/portconf"))
...
async def get_port_profile(
    ...
    response = await client.get(client.legacy_path(site.name, f"rest/portconf/{profile_id}"))
    raw_profiles: list[dict[str, Any]] = (
        response if isinstance(response, list) else response.get("data", [])
    )
...
async def create_port_profile(
    ...
    try:
        ...
        existing_response = await client.get(client.legacy_path(site.name, "rest/portconf"))
        ...
        for profile in existing_profiles:
            if profile.get("name") == name:
                raise DuplicateResourceError("port_profile", name, profile.get("_id", "unknown"))
        ...
        response = await client.post(
            client.legacy_path(site.name, "rest/portconf"), json_data=profile_data
        )
...
async def update_port_profile(
    ...
    try:
        ...
        response = await client.put(
            client.legacy_path(site.name, f"rest/portconf/{profile_id}"),
...
async def delete_port_profile(
    ...
    try:
        ...
        await client.delete(client.legacy_path(site.name, f"rest/portconf/{profile_id}"))
...
async def get_device_port_overrides(
    ...
    response = await client.get(client.legacy_path(site.name, "stat/device"))
...
async def set_device_port_overrides(
    ...
    try:
        ...
        endpoint = client.legacy_path(site.name, f"rest/device/{resolved_id}")
        response = await client.put(
            endpoint,
            json_data=device,
        )
...
async def get_device_by_mac(
    ...
    response = await client.get(client.legacy_path(site.name, f"stat/device/{mac}"))
